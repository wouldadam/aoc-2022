...
impl ListItem {
    fn list() -> RcListItem {
        Rc::new(RefCell::new(ListItem::List(vec![])))
    }
    ...
    fn add(&mut self, item: RcListItem) {
        match self {
            ...
            ListItem::List(ref mut list) => list.push(item),
        }
    }
    ...
    fn last_list(&mut self) -> RcListItem {
        match self {
            ...
            ListItem::List(ref mut list) => list.last().unwrap().clone(),
        }
    }

    fn parse(s: &str) -> RcListItem {
        let chars = s.chars().collect::<Vec<_>>();
        let top = ListItem::list();
...
fn ordering(left: RcListItem, right: RcListItem) -> std::cmp::Ordering {
    match (&*left.borrow(), &*right.borrow()) {
        ...
        (ListItem::Value(lv), ListItem::List(_)) => {
            let wrapper = Rc::new(RefCell::new(ListItem::List(vec![ListItem::value(*lv)])));
            ordering(wrapper, right.clone())
        }
        (ListItem::List(_), ListItem::Value(rv)) => {
            let wrapper = Rc::new(RefCell::new(ListItem::List(vec![ListItem::value(*rv)])));
            ordering(left.clone(), wrapper)
        }
        (ListItem::List(ll), ListItem::List(rl)) => {
            for idx in 0..ll.len() {
...
fn main() {
    ...
    let packet_pairs = input
        ...
        .map(|pair| (ListItem::parse(pair[0]), ListItem::parse(pair[1])))
        .collect::<Vec<_>>();

    // Sum the indices of correctly ordered packets
